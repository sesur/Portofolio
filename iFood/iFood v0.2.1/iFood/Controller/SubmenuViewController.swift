...
class SubmenuViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, Storyboarded {
    ...
    override func viewDidLoad() {
        ...
        submenuArray = getSubmenu(items: String(submenuItem))
        navigationItem.title = submenuItem
    }
    
    func getSubmenu(items: String) -> [Recipe] {
       return DataSet().getRecipes(forCategoryTitle: (Categories(rawValue: items))!)
    }
